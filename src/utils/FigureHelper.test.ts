...
test("Разделение кривой", () => {
  expect(
    ...
    [200, 300, 150, 200, 225, 150, 300, 150, 0, 0.5],
    [300, 150, 375, 150, 450, 200, 400, 300, 0.5, 1],
  ]);
});

test("Поиск пересечения", () => {
  const int = FigureHelper.bezInt(
    [200, 300, 100, 100, 500, 100, 400, 300],
    [0, 0, 0, 0, 500, 500, 500, 500]
  );

  expect(int).toEqual({
    location: { x: 196.2091047083959, y: 196.4773178100586 },
    ta: 0.22119140625,
    tb: 0.427734375,
  });
});
